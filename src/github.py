...
class Github:
    ...
    def trigger_workflow(self, feed_id: str, url: str, *, proxy: bool = False, cookie: bool = False) -> int:
        logger.info(f"Triggering workflow for {feed_id}")
        api = f"https://api.github.com/repos/{self.repo}/actions/workflows/single.yml/dispatches"
        data = {"ref": "main", "inputs": {"id": feed_id, "url": url, "proxy": proxy, "cookie": cookie}}
